...
const Feature = () => {
  const { data, loading, error } = useFetch("/hotels"); //所有飯店

  return (
    <div className="flex gap-4">
      {data.map((item) => (
        <div key={item._id} className="cursor-pointer w-full">
          <img src={item.photos[0]} className="rounded-xl mb-2 w-full" alt="" />
          <div className="flex flex-col">
            ...
            <div>
              <span className="text-gray-400  block  mb-1">{item.address}</span>
              <p>
